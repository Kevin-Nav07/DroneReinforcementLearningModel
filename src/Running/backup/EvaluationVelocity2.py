"""CrazyFlie Evaluation v7 — Velocity_TiltRobustness_v4 (25° tilt, 12° max_roll/pitch, auto_landing=True)"""
import os, time
import numpy as np
import mujoco
...
if __name__ == "__main__":
    ...
    env = CrazyFlieEnvVelocity(
        ...
        init_tilt_max_deg=25,    # evaluate with tilt — set 0.0 for clean run
        obs_noise_std=0.0, obs_bias_std=0.0, action_noise_std=0.0,
        motor_scale_std=0.0, torque_bias_std=0.0, torque_gust_std=0.0,
        drag_lin_max=0.0, drag_quad_max=0.0, frame_skip_jitter=0,
        # Use start_z_min=start_z_max=1.8 — guard handles numpy uniform(x,x) edge case
        start_z_min=0.2, start_z_max=0.2)
        ...
        while not (terminated or truncated):
            ...
            if now - last_print >= 1.0:
                ...
                print(f"t={int(now-t0):3d}s | z={z:+.3f} vz={vz:+.3f} | "
                      ...
                      f"tilt={info.get('tilt_deg',0):.1f}° r={info.get('radius',0):.2f} att={info.get('att_scale',1):.2f} | R={ep_ret:.2f}")
