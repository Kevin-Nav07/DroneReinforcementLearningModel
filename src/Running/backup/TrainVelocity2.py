...
class DebugCallback(BaseCallback):
    ...
    Every N steps runs two deterministic suites:
      ...
      TILT  — 25° spawn tilt, heights ≥ 0.50m (tilt recovery quality)

    TILT suite deliberately excludes z < 0.50 (new tilt_min_z for v4) because
    at 25° lean the drone cannot arrest angular momentum before hitting the floor
    at low heights — those episodes produce only -1 crash rewards and corrupt
    the gradient. 0.50m gives sufficient clearance for recovery attempts.
    ...
    def __init__(self, xml_path, target_z, max_steps, base_params,
        ...
        # Tilt heights start at 0.50 (new tilt_min_z floor for v4) and go higher.
        # At 25° lean the drone needs at least 0.50m clearance to arrest angular
        # momentum — 0.30m spawns consistently terminated in ~70 steps in v3.
        self.tilt_heights  = [0.50, 0.75, 1.00, 1.30, 1.50][:n_ep]
...
if __name__ == "__main__":
    ...
    models_dir = os.path.join(PROJECT_ROOT, "models", "Velocity_TiltRobustness_v4")
    logs_dir   = os.path.join(PROJECT_ROOT, "logs",   "Velocity_TiltRobustness_v4")
    ...
    SHARED = dict(
        ...
        auto_landing          = False,
          # random 0–25° lean at spawn (was 5° in v2)
        tilt_min_z            = 0.50,  # raised from 0.30 — at 25° lean the drone
                                       # needs more vertical clearance to arrest
                                       # angular momentum before hitting the floor
        obs_noise_std         = 0.0,
        obs_bias_std          = 0.0,
    ...
    EVAL = dict(**SHARED,
                init_tilt_max_deg=0.0,   # CRITICAL: override SHARED's 25° so eval is
                                         # always clean — without this, best_model
                                         # selection is noisy (reward varies with lean)
    ...
    # Raised entropy floor 0.001→0.005 and start 0.01→0.02.
    # v3 logs showed std collapsed monotonically from 1.01 to 0.29 — the 0.001
    # floor provides near-zero exploration pressure in the second half of training.
    # 0.005 keeps std above ~0.45 which is the minimum needed for tilt recovery
    # at different lean directions (observed from v3 debug callback data).
    ENT_START = 0.02
    ENT_END   = 0.005
    ...
    sep = "=" * 72
    print(f"\n{sep}")
    print("  CrazyFlie PPO — Velocity_TiltRobustness_v4 (from scratch)")
    print(f"  DR: init_tilt=25°  tilt_min_z=0.50  max_roll/pitch=12°  all other DR off")
    print(f"  {TOTAL_STEPS:,} steps | LR=1e-4→2e-5 | clip=0.20→0.05 | kl=0.02")
    print(f"  ent_coef=0.02→0.005 (higher floor — v3 showed std collapse to 0.29)")
    print(f"  Debug: CLEAN [0.01–1.00m] + TILT25 [0.50–1.50m] every 500k")
    ...
    model.save(os.path.join(models_dir, "tilt25_v4_final.zip"))
